...
<title>Welcome to Connectmu - Educational Network for Mumbai University</title>
 ...
 <script>
     $(document).ready(function () {
         $("#result").hide();
         var news_option = $("#feed_option").val();
         $("#feed_option").change(function () {
             var optionSelected = $(this).find("option:selected");
             news_option = optionSelected.val();
         });
         var autorefresh = setInterval(function () {
             $.post('loadfeeds.php', { news_option: news_option }, function (res) {
                 $("#content").html(res);
                 e.preventDefault();
...
<span id="account_option">
    <select id="feed_option" style="font-family: 'Segoe UI';font-size: 18px;">
